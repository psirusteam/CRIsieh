...
```{r}
df_censo <- df_censo %>% select(areageo2 = ID_ZONA, UPM2, ID_REGION, Hombre,
                              P03_EDAD) %>% collect() %>%
  mutate(region = ifelse(ID_REGION %in% 1:2, 1,
                              ifelse(ID_REGION == 3, 2, 
                              ifelse(ID_REGION == 4, 3,
                              ifelse(ID_REGION == 5, 4, 
                              ifelse(ID_REGION == 6, 5, 6))))),
                              sexo  = ifelse(is.na(Hombre), 2,
                                                    1),
                              edadR = ifelse(P03_EDAD < 15, 1,
                                       ifelse(P03_EDAD < 30, 2,
                                       ifelse(P03_EDAD < 45, 3,
                                       ifelse(P03_EDAD < 60, 4,
                                       ifelse(P03_EDAD >= 60, 5, NA)))))                                              )
                                
```

```{r}
M <-  length(unique(df_censo$UPM2))
```
...
# df_encuesta$edadR <- cut(df_encuesta$edad, breaks = c(0, 15, 30, 45, 60, max(df_encuesta$edad) + 1),
...
Cargamos funciones para apoyar la estimación de las tablas de muestreo por dominios

```{r}
source("funciones_estimaciones_parametros_V2.R", encoding = "UTF-8")
```
...
```{r, warning=FALSE, message=FALSE}
# UPM 5.251 no existe 
con_numUpmXzona <- df_censo %>% group_by(areageo2, UPM2) %>% summarise(num_personas = n()) %>% 
  group_by(areageo2) %>% summarise(num_upms = n()) %>% collect()
...
df_estima_indicadores <- f_indicadores_str_dominio("areageo2") 
```
...
```{r, warning=FALSE, message=FALSE}
df_estima_indicadores$Area <- as_factor(df_estima_indicadores$areageo2) %>% 
                              as.character()
...
tipologias_DesocOcupPartic <- data.frame(col_keys = colnames(TablaEditada_DesocOcupPartic),
                         what = c("Estimadores", "Tasa desocupacion", "Tasa desocupacion",                                "Tasa ocupacion", "Tasa ocupacion",
                                  "Tasa participacion", "Tasa participacion"),
                         measure = c("Estimadores", "Urbano", "Rural", "Urbano", "Rural",
                                                 "Urbano", "Rural"))  
...
```{r}
df_ParamGenerales <- f_paramGenerales_dominio("areageo2")
```
...
M_Urbana <- con_numUpmXzona$num_upms[con_numUpmXzona$areageo2 == 1]
M_rural <- con_numUpmXzona$num_upms[con_numUpmXzona$areageo2 == 2]
...
r_EdadTrabajar <- sum(df_encuesta$poblacion_EdadTrabajar2 *                                    df_encuesta$factorex, na.rm = T) /
                        sum(df_encuesta$factorex, na.rm = T)

# Temporar (hacerlo por dominio)
r_activos <- sum(df_encuesta$activos *                
                         df_encuesta$factorex, na.rm = T) /
                        sum(df_encuesta$factorex, na.rm = T)
...
A continuación estimamos el coeficiente de correlación intraclásica para la pobreza monetaria y el IPM con la encuesta de hogares de Costa Rica del año 2019:
...
Para elaborar las tablas de muestreo utilizaremos las medidas de precisión definidas por el INE de Costa Rica mediante sesiones de grupos focales. Las estimaciones puntuales se estimana a partir de la encuesta.
...
# tasa de desocupación fijado más baja que la estimación de la encuesta 2019
...
```{r}
# tablaMuestreo_tasa_desocupacion_Urbana <- ss4HHSp(N = df_ParamGenerales$N,
# M = M, 
# r = df_ParamGenerales$r_activos[1], 
# b = df_ParamGenerales$b,
# rho = df_estima_indicadores$tasa_desocupacion_rho[1],
# P = df_ValoresFijados$tasa_desocupacion_prop[indice_Urbana], 
# delta = df_ValoresFijados$tasa_desocupacion_mer[indice_Urbana],
# conf = 0.95, m = 5:15)
# 
# tablaMuestreo_tasa_desocupacion_Rural <- ss4HHSp(N = df_ParamGenerales$N,
# M = M, 
# r = df_ParamGenerales$r_activos[2], 
# b = df_ParamGenerales$b,
# rho = df_estima_indicadores$tasa_desocupacion_rho[2],
# P = df_ValoresFijados$tasa_desocupacion_prop[indice_Urbana], 
# delta = df_ValoresFijados$tasa_desocupacion_mer[indice_Urbana],
#conf = 0.95, m = 5:15)
```
...
```{r}
f_costos <- function(tablaMuestreo_Urbana, tablaMuestreo_Rural){
...
resultado <- list(tablaMuestreo_Urbana, tablaMuestreo_Rural)
names(resultado) <- c("tablaMuestreo_Urbana", "tablaMuestreo_Rural")
resultado
}
```
...
tablaMuestreo_tasa_desocupacion_Urbana <-  f_costos(tablaMuestreo_Urbana = tablaMuestreo_tasa_desocupacion_Urbana, 
         tablaMuestreo_Rural = tablaMuestreo_tasa_desocupacion_Rural)$tablaMuestreo_Urbana

tablaMuestreo_tasa_desocupacion_Rural <- f_costos(tablaMuestreo_Urbana = tablaMuestreo_tasa_desocupacion_Urbana, 
         tablaMuestreo_Rural = tablaMuestreo_tasa_desocupacion_Rural)$tablaMuestreo_Rural
```
...
tablaMuestreo_tasa_ocupacion_Urbana <-  f_costos(tablaMuestreo_Urbana = tablaMuestreo_tasa_ocupacion_Urbana, 
         tablaMuestreo_Rural = tablaMuestreo_tasa_ocupacion_Rural)$tablaMuestreo_Urbana

tablaMuestreo_tasa_ocupacion_Rural <- f_costos(tablaMuestreo_Urbana = tablaMuestreo_tasa_ocupacion_Urbana, 
         tablaMuestreo_Rural = tablaMuestreo_tasa_ocupacion_Rural)$tablaMuestreo_Rural
...
tablaMuestreo_tasa_participacion_Urbana <-  f_costos(tablaMuestreo_Urbana = tablaMuestreo_tasa_participacion_Urbana, 
         tablaMuestreo_Rural = tablaMuestreo_tasa_participacion_Rural)$tablaMuestreo_Urbana

tablaMuestreo_tasa_participacion_Rural <- f_costos(tablaMuestreo_Urbana = tablaMuestreo_tasa_participacion_Urbana, 
         tablaMuestreo_Rural = tablaMuestreo_tasa_participacion_Rural)$tablaMuestreo_Rural
...
tablaMuestreo_pobreza_Urbana <- f_costos(tablaMuestreo_Urbana = tablaMuestreo_pobreza_Urbana, 
         tablaMuestreo_Rural = tablaMuestreo_pobreza_Rural)$tablaMuestreo_Urbana

tablaMuestreo_pobreza_Rural <- f_costos(tablaMuestreo_Urbana = tablaMuestreo_pobreza_Urbana, 
         tablaMuestreo_Rural = tablaMuestreo_pobreza_Rural)$tablaMuestreo_Rural
```
...
tablaMuestreo_pobrezaExtr_Urbana <- f_costos(tablaMuestreo_Urbana = tablaMuestreo_pobrezaExtr_Urbana, 
         tablaMuestreo_Rural = tablaMuestreo_pobrezaExtr_Rural)$tablaMuestreo_Urbana

tablaMuestreo_pobrezaExtr_Rural <- f_costos(tablaMuestreo_Urbana = tablaMuestreo_pobrezaExtr_Urbana, 
         tablaMuestreo_Rural = tablaMuestreo_pobrezaExtr_Rural)$tablaMuestreo_Rural
...
tablaMuestreo_NBI_Urbana <- f_costos(tablaMuestreo_Urbana = tablaMuestreo_NBI_Urbana, 
         tablaMuestreo_Rural = tablaMuestreo_NBI_Rural)$tablaMuestreo_Urbana

tablaMuestreo_NBI_Rural <- f_costos(tablaMuestreo_Urbana = tablaMuestreo_NBI_Urbana, 
         tablaMuestreo_Rural = tablaMuestreo_NBI_Rural)$tablaMuestreo_Rural
...
```{r}
lista_resultados_tasa_desocupacion <- list(tablaMuestreo_tasa_desocupacion_Urbana, tablaMuestreo_tasa_desocupacion_Rural)
names(lista_resultados_tasa_desocupacion) <- c("tablaMuestreo_tasa_desocupacion_Urbana", "tablaMuestreo_tasa_desocupacion_Rural")

lista_resultados_tasa_ocupacion <- list(tablaMuestreo_tasa_ocupacion_Urbana, tablaMuestreo_tasa_ocupacion_Rural)
names(lista_resultados_tasa_ocupacion) <- c("tablaMuestreo_tasa_ocupacion_Urbana", "tablaMuestreo_tasa_ocupacion_Rural")


lista_resultados_tasa_participacion <- list(tablaMuestreo_tasa_participacion_Urbana, tablaMuestreo_tasa_participacion_Rural)
names(lista_resultados_tasa_participacion) <- c("tablaMuestreo_tasa_participacion_Urbana", "tablaMuestreo_tasa_participacion_Rural")

lista_resultados_pobreza <- list(tablaMuestreo_pobreza_Urbana, tablaMuestreo_pobreza_Rural)
names(lista_resultados_pobreza) <- c("tablaMuestreo_pobreza_Urbana", "tablaMuestreo_pobreza_Rural")

lista_resultados_pobrezaExtr <- list(tablaMuestreo_pobrezaExtr_Urbana, tablaMuestreo_pobrezaExtr_Rural)
names(lista_resultados_pobrezaExtr) <- c("tablaMuestreo_pobrezaExtr_Urbana", "tablaMuestreo_pobrezaExtr_Rural")

lista_resultados_NBI <- list(tablaMuestreo_NBI_Urbana, tablaMuestreo_NBI_Rural)
names(lista_resultados_NBI) <- c("tablaMuestreo_NBI_Urbana", "tablaMuestreo_NBI_Rural")
...
names(lista_resultados_tasa_desocupacion) <- c("Urbana", "Rural")
names(lista_resultados_tasa_ocupacion) <- c("Urbana", "Rural")
names(lista_resultados_tasa_participacion) <- c("Urbana", "Rural")
names(lista_resultados_pobreza) <- c("Urbana", "Rural")
names(lista_resultados_pobrezaExtr) <- c("Urbana", "Rural")
names(lista_resultados_NBI) <- c("Urbana", "Rural")
...
Estimaciones pobreza a nivel de región paso a paso:


```{r}
cubrimiento_panel <- 24 / 15
n_urbano = cubrimiento_panel * 8323
n_rural = cubrimiento_panel* 7718
str_dominio = "region"
str_indicador = "pobreza" 
r_indicador = "r_pobreza"

consulta_N_urbano <- df_censo %>% filter(areageo2 == 1) %>% group_by(region) %>%
  summarise(Nh = n())

consulta_N_rural <- df_censo %>% filter(areageo2 == 2) %>% group_by(region) %>%
  summarise(Nh = n())

# Repartición proporcional
# Hacer repartición de KISH !!
...
lst_me_region <- vector(mode = "list", length = length(unique(df_encuesta$region)))

for(i in 1:length(unique(df_encuesta$region))){
  
lst_me_region[[i]] <- me_calculo(N = f_paramGenerales_dominio("region")$N[i],
            n = nh[i], 
...
Realizamos la estimación de pobreza a nivel de área paso a paso:


```{r}
n_urbano = cubrimiento_panel * 8323
n_rural = cubrimiento_panel* 7718
str_dominio = "areageo2"
str_indicador = "pobreza" 
r_indicador = "r_pobreza"

consulta_N_urbano <- df_censo %>% filter(areageo2 == 1) %>% group_by(areageo2) %>%
  summarise(Nh = n())

consulta_N_rural <- df_censo %>% filter(areageo2 == 2) %>% group_by(areageo2) %>%
  summarise(Nh = n())
...
lst_me_region <- vector(mode = "list", length = length(unique(df_encuesta$areageo2)))

for(i in 1:length(unique(df_encuesta$areageo2))){
  
lst_me_region[[i]] <- me_calculo(N = f_paramGenerales_dominio("areageo2")$N[i],
            n = nh[i], 
            b = f_paramGenerales_dominio(str_dominio)$b[i], 
            r = f_paramGenerales_dominio("areageo2")$r_pobreza[i], 
            rho = f_indicadores_str_dominio(str_dominio)[[paste0(str_indicador,"_rho")]][i],
            P = f_indicadores_str_dominio(str_dominio)[[paste0(str_indicador,"_prop")]][i],
            conf = 0.95, m = 6)
}
...
Utilizaremos una función para reducir código anterior y realizar los demás análisis por dominios de interés:

```{r}
cubrimiento_panel <- 24 / 15

df_pobreza_region <- f_calculo_margen_error(n_urbano = cubrimiento_panel * 8323,
                                            n_rural = cubrimiento_panel* 7718,
                               str_dominio = "region",
                               str_indicador = "pobreza" ,
                               r_indicador = "r_pobreza")

df_pobreza_sexo <- f_calculo_margen_error(n_urbano = cubrimiento_panel * 8323, 
                                               n_rural = cubrimiento_panel* 7718,
                               str_dominio = "sexo",
                               str_indicador = "pobreza" ,
                               r_indicador = "r_pobreza")
...
df_pobreza_edad <- f_calculo_margen_error(n_urbano = cubrimiento_panel * 8323, 
                                               n_rural = cubrimiento_panel * 7718,
                               str_dominio = "edadR",
                               str_indicador = "pobreza" ,
                               r_indicador = "r_pobreza")

df_pobreza_area <- f_calculo_margen_error(n_urbano = cubrimiento_panel * 8323, 
                                               n_rural = cubrimiento_panel* 7718,
                               str_dominio = "areageo2",
                               str_indicador = "pobreza" ,
                               r_indicador = "r_pobreza")
...
df_pobrezaExtr_region <- f_calculo_margen_error(n_urbano = cubrimiento_panel * 8323,
                                            n_rural = cubrimiento_panel* 7718,
                               str_dominio = "region",
                               str_indicador = "pobrezaExtr" ,
                               r_indicador = "r_pobreza")

df_pobrezaExtr_sexo <- f_calculo_margen_error(n_urbano = cubrimiento_panel * 8323, 
                                               n_rural = cubrimiento_panel* 7718,
                               str_dominio = "sexo",
                               str_indicador = "pobrezaExtr" ,
                               r_indicador = "r_pobreza")
...
df_pobrezaExtr_edad <- f_calculo_margen_error(n_urbano = cubrimiento_panel * 8323, 
                                               n_rural = cubrimiento_panel* 7718,
                               str_dominio = "edadR",
                               str_indicador = "pobrezaExtr" ,
                               r_indicador = "r_pobreza")

df_pobrezaExtr_area <- f_calculo_margen_error(n_urbano = cubrimiento_panel * 8323, 
                                               n_rural = cubrimiento_panel* 7718,
                               str_dominio = "areageo2",
                               str_indicador = "pobrezaExtr" ,
                               r_indicador = "r_pobreza")
...
df_NBI_region <- f_calculo_margen_error(n_urbano = cubrimiento_panel * 8323,
                                            n_rural = cubrimiento_panel* 7718,
                               str_dominio = "region",
                               str_indicador = "NBI" ,
                               r_indicador = "r_pobreza")

df_NBI_sexo <- f_calculo_margen_error(n_urbano = cubrimiento_panel * 8323, 
                                               n_rural = cubrimiento_panel* 7718,
                               str_dominio = "sexo",
                               str_indicador = "NBI" ,
                               r_indicador = "r_pobreza")
...
df_NBI_edad <- f_calculo_margen_error(n_urbano = cubrimiento_panel * 8323, 
                                               n_rural = cubrimiento_panel* 7718,
                               str_dominio = "edadR",
                               str_indicador = "NBI" ,
                               r_indicador = "r_pobreza")
...
df_NBI_area <- f_calculo_margen_error(n_urbano = cubrimiento_panel * 8323, 
                                               n_rural = cubrimiento_panel* 7718,
                               str_dominio = "areageo2",
                               str_indicador = "NBI" ,
                               r_indicador = "r_pobreza")
...
# Tamaño de tasa de desocupación (Trimestral y Anual)
...
df_Desocupacion_region <- f_calculo_margen_error(n_urbano = cubrimiento_panel * 8323,
                                            n_rural = cubrimiento_panel* 7718,
                               str_dominio = "region",
                               str_indicador = "tasa_desocupacion" ,
                               r_indicador = "r_activos")

df_Desocupacion_sexo <- f_calculo_margen_error(n_urbano = cubrimiento_panel * 8323, 
                                               n_rural = cubrimiento_panel* 7718,
                               str_dominio = "sexo",
                               str_indicador = "tasa_desocupacion" ,
                               r_indicador = "r_activos")

df_Desocupacion_edad <- f_calculo_margen_error(n_urbano = cubrimiento_panel * 8323, 
                                               n_rural = cubrimiento_panel* 7718,,
                               str_dominio = "edadR",
                               str_indicador = "tasa_desocupacion" ,
                               r_indicador = "r_activos")

df_Desocupacion_area <- f_calculo_margen_error(n_urbano = cubrimiento_panel * 8323, 
                                               n_rural = cubrimiento_panel* 7718,
                               str_dominio = "areageo2",
                               str_indicador = "tasa_desocupacion" ,
                               r_indicador = "r_activos")
...
Estadísticas anuales de mercado de trabajo:
...
# Escenarios nacionales (panel)

```{r}
df_pobreza <- me_calculo(N = sum(df_ParamGenerales$N),
                                     n = 8323 + 7718, 
                                     b = (df_ParamGenerales$N[1] * df_ParamGenerales$b[1] +                                         df_ParamGenerales$N[2] * df_ParamGenerales$b[2]) /
             (df_ParamGenerales$N[1] + df_ParamGenerales$N[2]), 
                                     r = (df_ParamGenerales$N[1] * df_ParamGenerales$r_pobreza[1] +                                                          df_ParamGenerales$N[2] * df_ParamGenerales$r_pobreza[2]) /
             (df_ParamGenerales$N[1] + df_ParamGenerales$N[2]), 
                                     rho = f_indicadores_nacional()$pobreza_rho ,
                                     P =f_indicadores_nacional()$pobreza_prop,
                                     conf = 0.95, m = 6)
```
...
```{r}
df_pobrezaExtr <- me_calculo(N = sum(df_ParamGenerales$N),
                                     n = 8323 + 7718, 
                                     b = (df_ParamGenerales$N[1] * df_ParamGenerales$b[1] +                                         df_ParamGenerales$N[2] * df_ParamGenerales$b[2]) /
             (df_ParamGenerales$N[1] + df_ParamGenerales$N[2]), 
                                     r = (df_ParamGenerales$N[1] * df_ParamGenerales$r_pobrezaExtr[1] +                                                          df_ParamGenerales$N[2] * df_ParamGenerales$r_pobrezaExtr[2]) /
             (df_ParamGenerales$N[1] + df_ParamGenerales$N[2]), 
                                     rho = f_indicadores_nacional()$pobrezaExtr_rho ,
                                     P =f_indicadores_nacional()$pobrezaExtr_prop,
                                     conf = 0.95, m = 6)
```


```{r}
df_NBI <- me_calculo(N = sum(df_ParamGenerales$N),
                                     n = 8323 + 7718, 
                                     b = (df_ParamGenerales$N[1] * df_ParamGenerales$b[1] +                                         df_ParamGenerales$N[2] * df_ParamGenerales$b[2]) /
             (df_ParamGenerales$N[1] + df_ParamGenerales$N[2]), 
                                     r = (df_ParamGenerales$N[1] * df_ParamGenerales$r_NBI[1] +                                                          df_ParamGenerales$N[2] * df_ParamGenerales$r_NBI[2]) /
             (df_ParamGenerales$N[1] + df_ParamGenerales$N[2]), 
                                     rho = f_indicadores_nacional()$NBI_rho ,
                                     P =f_indicadores_nacional()$NBI_prop,
                                     conf = 0.95, m = 6)
```


```{r}
df_Desocupacion <- me_calculo(N = sum(df_ParamGenerales$N),
                                     n = 8323 + 7718, 
                                     b = (df_ParamGenerales$N[1] * df_ParamGenerales$b[1] +                                         df_ParamGenerales$N[2] * df_ParamGenerales$b[2]) /
             (df_ParamGenerales$N[1] + df_ParamGenerales$N[2]), 
                                     r = (df_ParamGenerales$N[1] * df_ParamGenerales$r_tasa_desocupacion[1] +                                              df_ParamGenerales$N[2] * df_ParamGenerales$r_tasa_desocupacion[2]) /
                                         (df_ParamGenerales$N[1] + df_ParamGenerales$N[2]), 
                                     rho = f_indicadores_nacional()$tasa_desocupacion_rho ,
                                     P= f_indicadores_nacional()$tasa_desocupacion_prop,
                                     conf = 0.95, m = 6)
```
